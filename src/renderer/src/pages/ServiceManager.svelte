<script lang="ts">
  ...
  import type { Service, MountState } from '../../../../contracts/types';
    ...
    async function handleUnmountAll() {
        if(confirm('¿Desmontar todos los servicios? Esto detendrá todas las conexiones activas.')) {
            try {
                const result = await window.api.invoke('mount:stop-all');
                if (result.success) {
                    // alert(`Se han desmontado ${result.unmountedCount} servicios.`);
                } else {
                    alert(`Algunos servicios no se pudieron desmontar:\n${result.errors.join('\n')}`);
                }
                await loadServices();
            } catch (e: any) {
                alert('Error al desmontar todos: ' + e.message);
            }
        }
    }
  ...
  <div class="p-4 min-h-screen relative pb-24">
    ...
    {:else}
        ...
        <div class="flex justify-end items-center mb-6">
            <button class="btn btn-circle btn-ghost text-error" onclick={handleUnmountAll} title="Desmontar todos">
        ...
        <dialog class="modal modal-open">
            <div class="modal-box text-center relative bg-white dark:bg-slate-800">
                ...
                <div class="flex flex-col items-center gap-4">
                    ...
                    <div>
                        <p class="text-lg font-semibold text-gray-500 dark:text-gray-400">Programado por</p>
